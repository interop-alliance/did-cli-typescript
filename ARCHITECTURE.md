...
[Testing](#testing)). The `was` and `did` groups are large enough that their
`run*` functions live in their own directories, `src/commands/was/` and
`src/commands/did/` (see the module map), while `src/commands/was.ts` and
`src/commands/did.ts` keep only their `makeXCommand()` wiring.
...
| `src/commands/was/`                           | The `was` group's `run*` functions, split by noun: `space`, `collection`, `resource`, `tree` (the `ls`/`rm` shorthands), `policy`, `publish` (publish/unpublish/grant), `shared` (option factories, address parsing, listing renderers), and `shell` (the `was shell` REPL, with its loop/dispatcher/session/tokenizer/completer under `src/commands/was/shell/`).                                                                                                                                                                                                                                     |
| `src/commands/did/`                           | The `did` group's `run*` functions: `create` (`runCreate`/`runAddKey` plus the seed/curve/output helpers and `DEFAULT_VERIFICATION_PURPOSES`), `service` (add/remove-service and `dispatchServiceUpdate`), `manage` (`runGet`/`runShow`/`runList`/`runMeta`/`runRemove`), and `webvh-update` (the did:webvh log/update-key plumbing, `parseDidLog`, `confirmAction`, and `runRotateKeys`).                                                                                          |
| `src/storage.ts`                              | Wallet + DID-document file I/O: `listCollection`, `loadFromCollection`, `saveToCollection`, `.meta.json` sidecars, and `methodOf` (a DID's method segment).                                                                                                                                                                                                                                                                                                                      |
...
| `src/was/`                                    | WAS client logic (non-CLI): `client`, `address`, `registry`, `capability`, `io`, `grant` (the `was grant` capability assembly/encoding/persistence).                                                                                                                                                                                                                                                                                                                                                                                                 |
...
di was shell [path]             interactive REPL over the was tree
```
